...
export function renderActivityDetail(vals, actions) {
  ...
  return el('div', { 'data-screen-label': 'Activity Detail', class: 'screen' }, [
    ...
    el('div', { style: { padding: '24px', marginTop: '-28px' } }, [
      ...
      el('div', { class: 'card', style: { borderRadius: '20px', padding: '22px', boxShadow: '0 12px 30px -20px rgba(60,50,30,.3)' } }, [
        ...
        el('div', { style: { marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '8px' } },
          a.materials.map((m) => el('div', { style: { background: '#F6F1E4', color: 'var(--text-muted-4)', font: "500 13px 'Nunito', sans-serif", padding: '6px 12px', borderRadius: '100px' } }, m))),
        ...
        el('div', { class: 'btn-primary', style: { marginTop: '20px', padding: '15px', borderRadius: '14px', font: "600 15px 'Quicksand', sans-serif" }, onClick: () => actions.goLogActivity() }, 'Log this activity'),
      ]),
    ]),
  ]);
}
